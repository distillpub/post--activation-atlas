...
a);this.refs={};this._state=r({root:"gcs-back",model:"inceptionv1",layerName:"mixed4d",grid:1,layout:0,gridSize:10,classHeatmap:-1,icons:[],clientWidth:45},a.data);this._recompute({clientWidth:1,model:1,layerName:1},this._state);this._intro=!0;this._handlers.update=[cg];this._fragment=dg(this,this._state);this.root._oncreate.push(()=>{ag.call(this);this.fire("update",{changed:J({},this._state),current:this._state})});a.target&&(this._fragment.c(),this._mount(a.target,
...
"True":"False"}--tile_size=${h.tile_size}--whiten=${h.whiten}--steps=${h.steps}--icon_size=${h.icon_size}--grid_size=${k}--layout=${h.layout[g]}--class_filter=${h.class_filter[e]}--filter=${h.filter[f]}--layer=${h.layer[d]}--model=${h.model}--sample_images=${h.sample_images}--sample_type=${h.sample_type}.jpg`;n[a][A]=m}m[A]=n});this.set({layers:m,loading:!1})}))}}function ge(a){x(this,a);this._state=r({fingerprint:Math.random()+Date.now(),root:"gcs-back",
