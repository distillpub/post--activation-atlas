...
a);this.refs={};this._state=r({root:"https://storage.googleapis.com/distill-2019-activation-atlas/build",model:"inceptionv1",layerName:"mixed4d",grid:1,layout:0,gridSize:10,classHeatmap:-1,icons:[],clientWidth:45},a.data);this._recompute({clientWidth:1,model:1,layerName:1},this._state);this._intro=!0;this._handlers.update=[cg];this._fragment=dg(this,this._state);this.root._oncreate.push(()=>{ag.call(this);this.fire("update",{changed:J({},this._state),current:this._state})});a.target&&(this._fragment.c(),
this._mount(a.target,a.anchor),E(this))}function Ud(a,b,c){a=Object.create(a);a.layer=b[c];a.i=c;return a}function eg(a,b){for(var c,d=b.layers,e=[],f=0;f<d.length;f+=1)e[f]=Vd(a,Ud(b,d,f));return{c(){c=k("div");for(var a=0;a<e.length;a+=1)e[a].c();c.className="stack"},m(a,b){a.insertBefore(c,b);for(a=0;a<e.length;a+=1)e[a].m(c,null)},p(b,f){if(b.layerName||b.layers||b.classHeatmap){d=f.layers;for(var g=0;g<d.length;g+=1){let h=Ud(f,d,g);e[g]?e[g].p(b,h):(e[g]=Vd(a,h),e[g].c(),e[g].m(c,null))}for(;g<
e.length;g+=1)e[g].d(1);e.length=d.length}},d(a){a&&v(c);jb(e,a)}}}function Vd(a,b){function c(){a.set({layerName:e.__value})}var d,e,f,g,h,l,p,m=b.layer,A,D,ba,Ua=new ub({root:a.root,store:a.store,data:{grid:0,layerName:b.layer,classHeatmap:b.classHeatmap}});return{c(){d=k("label");e=k("input");g=n("\n    ");h=k("div");Ua._fragment.c();l=n("\n    ");p=k("span");A=n(m);D=n("\n  ");a._bindingGroups[0].push(e);e.addEventListener("change",c,void 0);t(e,"type","radio");e.__value=f=b.layer;e.value=e.__value;
e.className="svelte-1lxnb2u";h.className="thumb svelte-1lxnb2u";q(h,"position","relative");p.className="svelte-1lxnb2u";d.className=ba="layer "+(b.layerName==b.layer?"selected":"")+" svelte-1lxnb2u"},m(a,c){a.insertBefore(d,c);d.appendChild(e);e.checked=e.__value===b.layerName;d.appendChild(g);d.appendChild(h);Ua._mount(h,null);d.appendChild(l);d.appendChild(p);p.appendChild(A);d.appendChild(D)},p(a,b){a.layerName&&(e.checked=e.__value===b.layerName);a.layers&&f!==(f=b.layer)&&(e.__value=f);e.value=
e.__value;var c={};a.layers&&(c.layerName=b.layer);a.classHeatmap&&(c.classHeatmap=b.classHeatmap);Ua._set(c);a.layers&&m!==(m=b.layer)&&(A.data=""+m);(a.layerName||a.layers)&&ba!==(ba="layer "+(b.layerName==b.layer?"selected":"")+" svelte-1lxnb2u")&&(d.className=ba)},d(b){b&&v(d);a._bindingGroups[0].splice(a._bindingGroups[0].indexOf(e),1);e.removeEventListener("change",c,void 0);Ua.destroy()}}}function Wd(a){x(this,a);this._state=r({layerName:"mixed4c",layers:"mixed3a mixed3b mixed4a mixed4b mixed4c mixed4d mixed4e mixed5a mixed5b".split(" "),
...
"True":"False"}--tile_size=${h.tile_size}--whiten=${h.whiten}--steps=${h.steps}--icon_size=${h.icon_size}--grid_size=${k}--layout=${h.layout[g]}--class_filter=${h.class_filter[e]}--filter=${h.filter[f]}--layer=${h.layer[d]}--model=${h.model}--sample_images=${h.sample_images}--sample_type=${h.sample_type}.jpg`;n[a][A]=m}m[A]=n});this.set({layers:m,loading:!1})}))}}function ge(a){x(this,a);this._state=r({fingerprint:Math.random()+Date.now(),root:"https://storage.googleapis.com/distill-2019-activation-atlas/build",
